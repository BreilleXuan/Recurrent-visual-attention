...
img = tf.placeholder(tf.float32, shape=(batch_size, 28 * 28), name="images")
labels = tf.placeholder(tf.float32, shape=(batch_size), name="labels")
tensor_labels = tf.placeholder(tf.float32, shape=(batch_size, 10), name="tensor_labels")
...
def glimpseSensor(img, normLoc):
    ...
    for k in xrange(batch_size):
        ...
        one_img = tf.image.pad_to_bounding_box(one_img, max_radius, max_radius, \
            max_radius * 4 + mnist_size, max_radius * 4 + mnist_size) # 8*2 + 28
...
def lable_pred(output):
    ...
    with tf.variable_scope("pred", reuse = DO_SHARE):
        pred_tensor = linear(output, n_classes)
...
mean_locs, sampled_locs = [0] * glimpses, [0] * glimpses
# initial state
...
for t in range(glimpses):
    ...
    if (t + 1) == glimpses:
        DO_SHARE = False
        pred_tensor, pred = lable_pred(output)  # batch_size * 1
    DO_SHARE = True
